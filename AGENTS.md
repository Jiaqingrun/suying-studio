...
- **默认不推远程 Git**（GitHub/GitLab 等）；异地副本走 **极空间 T2S** `QR备份/`（`qr t2s-backup`，详见 `~/QR/dev/qr/docs/T2S_WORKSPACE_BACKUP.md`）。
...
- **T2S 工作区备份**：`qr schedule install` 安装 `com.qr.t2s-backup`（默认每 4 小时）；**全部** `~/QR` 项目含 `dev/速影` 纳入极空间 `QR备份/`；`qr workspace new` 自动创建极空间项目目录；Cursor **执行-** 任务改代码后执行 `qr t2s-backup -p <项目>` 或 `qr t2s-backup --changed`（**不**默认 `git push`）。
...
> 教训：清理项目痕迹时误执行 `defaults delete com.apple.dock persistent-apps`，清空 Dock 应用图标。此后凡「删除 / 清理 / 卸载」任务必须遵守本节。

- **禁止**修改或删除 macOS **系统 UI / 偏好**，除非用户**逐项点名**该偏好域与键。尤其禁止：
  - `defaults delete com.apple.dock …`（含 `persistent-apps` / `persistent-others` / 整域删除）
  - `defaults delete` / `defaults write` 作用于 `com.apple.*`、登录项、键盘/触控板/显示/网络等系统域
  - `killall Dock` / `killall Finder` / `killall SystemUIServer` 作为「清缓存」顺手操作（用户明确要求刷新 Dock 图标时除外）
- **删除范围白名单**：只删用户**明确列出**或任务目标可直接推导的路径（如某项目目录、该 App bundle、该 App 的 `com.<app>.*` 偏好、声明过的数据根）。**禁止**「清理顺便」扩大到 Dock、Spotlight、其他 App、全局 LaunchAgents、无关同步盘。
- **命令自检**：执行任何 `rm` / `defaults delete` / 远端删除 API 前，核对路径或域名是否落在白名单；不确定则**停手询问**，不得猜测删除。
- **项目删除**优先走 `qr workspace delete`（或用户指定的项目卸载脚本）；其保留的时间线/引导语是否再清，须用户另说一句，不得默认扩大。
- **验收**：破坏性任务结束后，不得留下「顺手改过系统偏好」的副作用；若发现越界，立即停止并如实说明。
...
>
> **Agent 入口（单一指向）**：[`docs/README.md`](docs/README.md) → [`docs/DEV_LOCK.md`](docs/DEV_LOCK.md) → [`docs/HARD_LOCKS.md`](docs/HARD_LOCKS.md) → 分锁。冲突裁决：`DEV_LOCK` / `HARD_LOCKS` > 分锁 > 索引说明 > 运维文 > `docs/archive/`。
...
- **HARD_LOCKS L17（冻结 · A7=A）**：`LangCombobox`（旁白/字幕语言）Portal+fixed 已验收；**含视觉皮肤/外观**，无授权不得再改该组件与 `.lang-combo*`；禁止卡片内 absolute 弹层与靠祖先 overflow「治裁切」；一体包须重建 `appsdesktop` 后再验
...
- **HARD_LOCKS L19（冻结 · 本地 F5 就绪与试听）**：**交付主路径 = core 一体包 + App 外 F5 Runtime Kit**（`~/Suying/runtime`）；clone flavor / 内嵌 `f5-tts` **仅应急**，非 T2S 周更默认；设置页试听 `POST /voice/packs/preview` 单轨 `preview.wav` + macOS `afplay` 出声已验收；细则 [`docs/VOICE_CLONE.md`](docs/VOICE_CLONE.md)。**无用户当面授权禁止再改** `voice_clone` 就绪语义、预览接口、设置页音色试听相关实现
