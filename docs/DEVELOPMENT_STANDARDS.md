...
| Tab | 职责 | 是否客户相关 |
|-----|------|----------------|
| 总览 | 健康、今日配额、ready 率、告警 | 当前客户 |
| 生产 | 日历、dry-run、任务、熔断 | 当前客户 |
| 素材 | 扫描、列表、向量化缺口 | 当前客户 |
| 审片 | 通过/打回（原因码） | 当前客户 |
| 物料 | publish_pack 预览/导出（Phase 2） | 当前客户 |
| 触达 | 发布队列、消息提醒（Phase 4） | 当前客户 |
| 运维 | 路径、同步、磁盘、引擎启停、**客户切换/新建** | 机器级 + 客户级 |
...
- 单文件 `App.tsx` 过胖时按 Tab 拆页面组件；**状态仍以 API 为准**，不在前端持久化业务库。  
