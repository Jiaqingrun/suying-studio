...
> 全量清单：[`APP_INTERVAL_INVENTORY.md`](APP_INTERVAL_INVENTORY.md) · CI：`scripts/check-app-poll-budget.sh`。

> 冲突时：`DEV_LOCK.md` / `HARD_LOCKS.md` > 本文。权威索引见 [`README.md`](README.md)。
| 通道 | 间隔 | 接口 | 说明 |
|------|------|------|------|
| health | **5s** | `GET /health`（经 Rust `engine_status` 单次 `/readiness`） | 主心跳；勿在其它 effect 再叠一层 |
| jobs | **5s** | `GET /jobs` | 与 health 同 tick，勿单独 ≤1s |
| services | **~30s** | `GET /ops/services` | **勿**与 health 1:1；运维页开关后即时刷新 |
...
| pipeline（忙） | **3s** | `GET /jobs/pipeline` | 生产页有 running/queued |
| pipeline（闲） | **15s** | `GET /jobs/pipeline` | 生产页空闲 |
