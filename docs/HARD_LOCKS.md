...
| **L20** | [`SERVICE_NARRATION_LOCK.md`](SERVICE_NARRATION_LOCK.md) + `engine/pack/ollama_narration.py` / `narration_script.py` / `copy_diversity_gate.py` | **生活服务旁白（L20 v2）**：装配仍遵服务禁词与倾听口径；Ollama = 受控多样改写 + 近窗去重；`use_golden_base_lock` 关闭为 v2 授权态；无新授权禁止回退黄金硬锁 |
...
23. **生活服务旁白（L20 v2）**：装配底稿仍遵服务禁词与倾听口径（禁「咱们/聊聊天」、禁「请听」代替「倾听」、禁「安排到店服务 / 来之前需要怎样安排 / 会提前和您讲清楚」等空垫）。Ollama = **受控多样改写** + 近窗去重（`copy_diversity_gate`）；`use_golden_base_lock` **关闭**为 v2 授权态；无新授权禁止回退黄金硬锁。短句呼吸与 `rate=-8%` 仍服从 L2。2026-08-09 听感冻结；2026-08-29 授权 v2；细则 [`SERVICE_NARRATION_LOCK.md`](SERVICE_NARRATION_LOCK.md)。
...
| 2026-09-16 | **A1=A** · 底线 §23 与分锁表 L20 对齐 `SERVICE_NARRATION_LOCK` v2（删「仅节奏润色」过时句）；裁决见遗留清理计划 A1 |
